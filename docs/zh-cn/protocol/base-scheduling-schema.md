...
在任务参数中设置 `mode = 20000`，并指定 `rotation_style = "station_preset"`，同时提供 `filename` 与 `plan_index`（或 GUI 按 `period` 自动选择）。JSON 中每个 plan 使用 `preset` 对象即可，**无需**填写 `strategy`。
...
- GUI 中的“宿舍空余位置蹭信赖”“不将已进驻的干员放入宿舍”“源石碎片自动补货”“会客室信息板收取信用”“进行线索交流”“赠送线索”“训练完成后继续尝试专精当前技能”设置仍会生效。会客室收线索默认执行，无需单独开关。启用源石碎片自动补货时，MAA 会额外进入制造站逐个识别产物并补货，但不会执行逐干员换班。
