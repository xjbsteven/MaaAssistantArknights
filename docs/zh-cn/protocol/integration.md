...
<br>
`8` - 刷目标藏品（当前仅 Mizuki）：选关策略与模式 0 相同，打到失败或通关；进诡意行商按列表 OCR 购买并骰子刷新，无货后离店继续；战后藏品几选一优先点列表中的目标。
:::  
...
刷藏品目标列表，顺序即优先级。仅在模式 `8` 时有效且必填。  
<br>
进诡意行商后按列表 OCR 购买；找不到则刷新；刷新用尽仍没有则离店继续推进。  
<br>
战后藏品几选一时，按同一列表优先点选；列表中均未出现则回退为原「选择」按钮匹配。  
<br>
出现几选一时，选择前会截图保存到用户目录下 `debug/roguelike/collectibleSelect/`（与商店截图目录平行，不自动清理）。  
<br>
