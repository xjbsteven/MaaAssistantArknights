...
std::pair<int, int> switch_button_search_y_range(const Rect& room_text_rect)
{
    ...
    const int y_end =
        std::min(SwitchButtonSearchRoi.y + SwitchButtonSearchRoi.height, room_text_rect.y + FacilityRowHeight + 20);
...
std::optional<Rect> geometry_switch_button_rect(const Rect& room_text_rect)
{
    ...
    return Rect(SwitchButtonLeftX, room_text_rect.y + SwitchButtonTopOffsetY, SwitchButtonWidth, SwitchButtonHeight);
}
...
bool is_section_header_ocr_text(const std::string& text)
{
    ...
    for (const auto& prefix : Prefixes) {
        ...
        if (pos > 0) {
            ...
            if (std::ranges::any_of(head, is_ascii_bar) || head.find(FullwidthBar) != std::string_view::npos) {
                return true;
            }
        }
        if (pos == 0 && !text.empty()) {
...
std::optional<Rect> find_enabled_switch_button_by_color(const cv::Mat& image, const Rect& room_text_rect)
{
    ...
    for (int y = y_begin; y < std::min(y_end, image.rows); ++y) {
        ...
        if (row_count >= 3) {
            if (!cur_run) {
                cur_run =
                    BlueRun { .y_begin = y, .y_end = y, .min_x = row_min_x, .max_x = row_max_x, .count = row_count };
            }
            else {
                cur_run->y_end = y;
    ...
    return Rect(
        best_iter->min_x,
        best_iter->y_begin,
        best_iter->max_x - best_iter->min_x + 1,
        best_iter->y_end - best_iter->y_begin + 1);
}
} // namespace

asst::InfrastPresetTask::InfrastPresetTask(const AsstCallback& callback, Assistant* inst, std::string_view task_chain) :
    ...
    for (int swipe_times = 0; !rooms.empty() && swipe_times <= MaxScrollTimes; ++swipe_times) {
        ...
        for (auto iter = rooms.begin(); iter != rooms.end();) {
            ...
            if (!button) {
                ...
                Log.warn(
                    "facility preset switch button not found, retry later:",
                    iter->id,
                    visible_iter->second.to_string());
                ++iter;
                continue;
            }
            ...
            ctrler()->click(
                Point(button->x + button->width / 2 + SwitchButtonClickOffsetX, button->y + button->height / 2));
            sleep(800);
            clicked = true;
    ...
    asst::InfrastPresetTask::analyze_visible_rooms(const cv::Mat& image, const std::vector<RoomInfo>& rooms) const
{
    ...
    for (auto& row : raw_rows) {
        if (!rows.empty() && std::abs(row.rect.y - rows.back().rect.y) < RowClusterY) {
            const bool better =
                std::pair(row_priority(row), row.score) > std::pair(row_priority(rows.back()), rows.back().score);
            if (better) {
                rows.back() = row;
            }
            continue;
...
std::optional<Rect>
    asst::InfrastPresetTask::find_enabled_switch_button(const cv::Mat& image, const Rect& room_text_rect) const
{
    ...
    const Rect search_roi(SwitchButtonSearchRoi.x, y_begin, SwitchButtonSearchRoi.width, y_end - y_begin);
    ...
    if (auto geometry_button = geometry_switch_button_rect(room_text_rect)) {
        ...
        if (verify_matcher.analyze() && !verify_matcher.get_result().empty()) {
            ...
            if (best_iter != matches.cend() && best_iter->score >= SwitchButtonGeometryVerifyThreshold) {
                if (auto button = accept_button(best_iter->rect)) {
                    Log.trace("facility preset switch geometry verified:", best_iter->score, button->to_string());
                    return button;
                }
            }
        }
